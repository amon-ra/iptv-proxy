    ...
    def __custom_sort(self, url):
        ...
        for key in self.args.sort_keys:
            ...
            if key == 'height':
                ...
                res.append(int(re.sub("\D", "", entry.get('params',{}).get(key,'')) or sys.maxsize))
            elif key == 'template-order':
                res.append(int(entry.get('params',{}).get(key,0) or sys.maxsize))
            elif key == 'group-title':
                res.append(entry.get('params',{}).get(key,'') or '')
        return res
